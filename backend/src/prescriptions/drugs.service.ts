...
export class DrugsService {
  ...
  async findAll(search?: string, category?: string, form?: string): Promise<Drug[]> {
  ...
  async update(id: string, dto: Partial<CreateDrugDto>): Promise<Drug> {
    const drug = await this.findOne(id);
    Object.assign(drug, dto);
    return this.drugRepo.save(drug);
  }

  async toggleActive(id: string): Promise<Drug> {
    const drug = await this.findOne(id);
